...
var toc = (function () {
  ...
  var $_c2xvesrkjgqkpwvf = { create: create };

  var tocId = $_c2xvesrkjgqkpwvf.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_4khjtrjjgqkpwve.getTocClass(editor);
    var headerTag = $_4khjtrjjgqkpwve.getTocHeader(editor);
    var selector = generateSelector($_4khjtrjjgqkpwve.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_4khjtrjjgqkpwve.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_4khjtrjjgqkpwve.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_b6ojw8rfjgqkpwv9.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_b6ojw8rfjgqkpwv9.updateToc(editor);
    });
  };
  var $_4v06ierejgqkpwv7 = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_4khjtrjjgqkpwve.getTocClass(editor);
  ...
  var $_41pkchrljgqkpwvg = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_b6ojw8rfjgqkpwv9.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_4khjtrjjgqkpwve.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_eaxkpdrmjgqkpwvi = { register: register$1 };

  global.add('toc', function (editor) {
    $_4v06ierejgqkpwv7.register(editor);
    $_eaxkpdrmjgqkpwvi.register(editor);
    $_41pkchrljgqkpwvg.setup(editor);
  });
  function Plugin () {
  }
