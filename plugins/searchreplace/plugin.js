...
var searchreplace = (function () {
  ...
  var $_g299ucjejgqkpun4 = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_g299ucjejgqkpun4.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_cymwghjcjgqkpumy.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_cymwghjcjgqkpumy.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_cymwghjcjgqkpumy.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_cymwghjcjgqkpumy.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_cymwghjcjgqkpumy.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_cymwghjcjgqkpumy.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_cymwghjcjgqkpumy.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_cymwghjcjgqkpumy.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_cymwghjcjgqkpumy.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_cymwghjcjgqkpumy.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_cymwghjcjgqkpumy.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_cymwghjcjgqkpumy.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_cymwghjcjgqkpumy.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_cymwghjcjgqkpumy.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_cymwghjcjgqkpumy.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_blsth1jgjgqkpuna = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_blsth1jgjgqkpuna.open(editor, currentIndexState);
    });
  };
  var $_anu3ckjfjgqkpun9 = { register: register };
  ...
  var $_bsfbgijhjgqkpund = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_anu3ckjfjgqkpun9.register(editor, currentIndexState);
    $_bsfbgijhjgqkpund.register(editor, currentIndexState);
    return $_3w25iwjbjgqkpumw.get(editor, currentIndexState);
  });
  function Plugin () {
  }
